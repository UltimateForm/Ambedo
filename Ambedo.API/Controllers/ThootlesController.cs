...
namespace Ambedo.API.Controllers
{
    ...
    public class ThootlesController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult<Dtos.Thootle>> Post([FromBody] Dtos.Thootle data)
        {
            var result = await _service.CreateThootle(_mapper.Map<Dtos.Thootle, Thootle>(data));
            return Ok(_mapper.Map<Thootle, Dtos.Thootle>(result));
        }

        [HttpGet]
        public async Task<ActionResult<IEnumerable<Dtos.Thootle>>> Get([FromQuery] Dtos.Thootle filter)
        {
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult<Dtos.ReplaceOneResult>> Put([FromRoute] string id, [FromBody] Dtos.Thootle data)
        {
            var result = await _service.UpdateThootle(id, _mapper.Map<Dtos.Thootle, Thootle>(data));
